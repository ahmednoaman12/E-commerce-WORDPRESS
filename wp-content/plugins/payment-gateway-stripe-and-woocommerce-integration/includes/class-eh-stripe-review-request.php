...
class Eh_Stripe_Review_Request
{
    /**
     * config options 
     */
    private $plugin_title               =   "Stripe Payment Gateway for WooCommerce";
    private $review_url                 =   "https://wordpress.org/support/plugin/payment-gateway-stripe-and-woocommerce-integration/reviews/";
    private $plugin_prefix              =   "wt_stripe"; /* must be unique name */
    private $activation_hook            =   "wt_stripe_activate"; /* hook for activation, to store activated date */
    private $deactivation_hook          =   "wt_stripe_deactivate"; /* hook for deactivation, to delete activated date */
    private $days_to_show_banner        =   8; /* when did the banner to show */
    private $remind_days                =   8; /* remind interval in days */
    private $webtoffee_logo_url         =   EH_STRIPE_MAIN_URL_PATH . 'assets/img/webtoffee-logo_small.png';
    ...
    public function __construct()
    {
        ...
        if ($this->check_condition()) /* checks the banner is active now */ {
            $this->banner_message = sprintf(__("Hey, we at %sWebToffee%s would like to thank you for using our plugin. We would really appreciate if you could take a moment to drop a quick review that will inspire us to keep going.", 'payment-gateway-stripe-and-woocommerce-integration'), '<b>', '</b>');

            /* button texts */
            $this->later_btn_text   = __("Remind me later", 'payment-gateway-stripe-and-woocommerce-integration');
            $this->never_btn_text   = __("Not interested", 'payment-gateway-stripe-and-woocommerce-integration');
            $this->review_btn_text  = __("Review now", 'payment-gateway-stripe-and-woocommerce-integration');
